...
python pipeline.py --data data --out output
```

`data` — папка с `nodes.parquet`, `edges.parquet`, `transactions.parquet`. Полный пересчёт занимает 10–20 секунд (лимит ТЗ — 5 минут). Команда возвращает код 0, если все проверки выгрузок пройдены.

Windows (Anaconda Prompt), путь с пробелами берётся в кавычки:

```bat
cd "C:\path\to\repo"
python pipeline.py --data "C:\path\to\data" --out output
```

Дополнительно:

```bash
python pipeline.py --data data --out output --card <gid>          # карточка узла в консоли
python assumption_checks.py --data data --out output               # проверки допущений и устойчивости (~1 мин)
python -m pytest                                                   # тесты (~1 мин; реальные данные — через GRAPH_DATA)
GRAPH_DATA=data python -m pytest                                   # Windows: set GRAPH_DATA=data && python -m pytest
```

На Windows всё сразу (расчёт, проверки, тесты): `run_all.bat "путь\к\data"`.

Из Jupyter:

```python
import pipeline as pl
R = pl.run("data", "output")
...
1. `python pipeline.py --data data --out output` — живой прогон, 10–20 с, все проверки OK.

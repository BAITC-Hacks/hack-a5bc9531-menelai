...
1. **Пайплайн** (`pipeline/run.py`, Python) читает три `.parquet` из `task/data/`, считает метрики узлов (степени, суммы, коэффициент пропуска, seed-деньги, посредничество, циклы, быстрый вывод, синхронные входы), каскадом правил с явными порогами назначает одну из 6 ролей, находит кластеры (Louvain, на неориентированном графе) и считает `priority_score`. Пересчёт от parquet до CSV — одна команда, несколько секунд:

   ```bash
   cd pipeline && uv sync && uv run python run.py
   ```

2. **Выход** в `out/`: `nodes_roles.csv` (ровно 2 248 строк и 6 колонок схемы ТЗ: `gid, role, role_score, cluster_id, priority_score, evidence`), `clusters.csv`, `top_nodes.csv`; все метрики узлов — в `node_metrics.csv` и `node_metrics.json`, метрики рёбер — в `edge_metrics.csv`.
3. **Сервер** (Bun + Hono) читает parquet и `out/` один раз при старте и отдаёт граф, роли, кластеры и аналитику по HTTP API.
4. **Клиент** (React) показывает результат: обзор, схему сети, приоритеты, кластеры, карточку узла, аналитику, AI-ассистента, правила ролей, схему решения и скачивание трёх CSV. Клиент и сервер роли не пересчитывают.
...
client/src/
  ...
  pages/          Обзор, Схема сети, Карточка узла, Приоритеты, Кластеры, анализ (Маршруты, Время, Аномалии,
                  Устойчивость, Полнота), Ассистент, метод (Правила ролей, Схема решения), 404
...
Маршруты top, clusters, meta, export и analytics возвращают 503, если в `out/` нет выгрузки пайплайна. Неизвестный путь под `/api/` — JSON `{ "error": "not found" }` с кодом 404.
...
https://monelai.up.railway.app — один сервис Railway: `bun run build` собирает клиент, `bun run start` запускает сервер, который отдаёт `/api` и `client/dist`. Деплой из локальной копии: `railway up --service graf-deneg` (репозиторий организации не подключён к Railway, автодеплоя по push нет).

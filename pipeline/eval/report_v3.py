...
if len(el):
    ...
    L += ["Замечаний нет."]
L += ["", "## 4. Сравнение с v1", "",
      "v1 (54 gid выборки, RULES v1, `snapshot_v1`): 54 «да», 12 «частично», 0 «нет». "
      f"v3 (65 gid выборки, RULES v3, `snapshot_v3`): {(t.verdict == 'да').sum()} «да», {(t.verdict == 'частично').sum()} «частично», "
      f"{(t.verdict == 'нет').sum()} «нет».", "",
      pd.DataFrame({"v1 (n=54)": t1.verdict.value_counts(), "v3 (n=65)": t.verdict.value_counts()}).fillna(0).astype(int).to_markdown(), "",
      "Прочитение: доля «частично» упала с 12/54 (22 %) до 0/65 — согласуется с правками v2→v3, которые жюри-критика (задача 1, v1) "
